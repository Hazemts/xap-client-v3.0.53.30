    ...
    void RenderUI() {
        if (ImGui::BeginTabItem("Sense", nullptr, ImGuiTabItemFlags_NoCloseWithMiddleMouseButton | ImGuiTabItemFlags_NoReorder)) {
            ...
            if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled))
                ImGui::SetTooltip("Enable if playing Control/TDM, Disable if playing BR/Gun Run/Training Area");
    ...
    void RenderDrawings(ImDrawList *Canvas, Aimbot *AimAssistState, LocalPlayer *Myself, Overlay OverlayWindow)
    {
        ...
        for (int i = 0; i < Players->size(); i++)
        {
            ...
            if (p->IsCombatReady() && p->IsHostile && p->DistanceToLocalPlayer < (Conversion::ToGameUnits(ESPMaxDistance)))
            {
                ...
                if (DrawTracers)
                {
                    ...
                    if (!chestScreenPosition.IsZeroVector())
                    {
                        ...
                        Renderer::DrawLine(Canvas, Vector2D(x, ScreenHeight), chestScreenPosition, 1.5f, ImColor(TracerColor));
                        Renderer::DrawCircleFilled(Canvas, chestScreenPosition, 2, 10, ImColor(TracerColor));
                    }
                }
            ...
            if (DrawNames && !p->IsDummy()) {
                    ...
                    Renderer::DrawText(Canvas, originScreenPosition.Add(Vector2D(0, 0)), p->GetPlayerName().c_str(), ImVec4(1.0, 1, 1.0, 1), true, true, false);
                }
           //}
                //Draw Skeleton
                if (Skeleton)
                {
                    ...
                    // Renderer::DrawCircle(Canvas, Head, 10.0f, 10, SkeletonColor, SkeletonThickness);
            ...
            if (!p->IsCombatReady() || !p->IsVisible || !p->IsHostile || p->DistanceToLocalPlayer > (Conversion::ToGameUnits(ESPMaxDistance)) || Myself->BasePointer == p->BasePointer)
                continue;
            // Tracer OLD HIRE
